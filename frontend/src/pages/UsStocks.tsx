...
function parseRoute(): RouteState {
  const match = window.location.pathname.match(/^\/us-stocks\/stocks\/([A-Za-z0-9.-]+)$/);
...
function DashboardPage() {
  ...
  const openStock = (ticker: string) =>
    navigateTo(`/us-stocks/stocks/${ticker}?date=${ranking?.as_of_date ?? asOfDate}`);

  return (
    <main className="us-stocks-app">
...
function StockDetailPage({ ticker, initialDate }: { ticker: string; initialDate: string }) {
  ...
  const updateDate = (date: string) => {
    setAsOfDate(date);
    window.history.replaceState({}, "", `/us-stocks/stocks/${ticker}?date=${date}`);
  };

  return (
    <main className="us-stocks-app detailPage">
      <header className="topbar">
        <div>
          <button className="ghostButton" type="button" onClick={() => navigateTo("/us-stocks")}>
